...
def print_gpu_memory():
    print(f"Allocated: {torch.cuda.memory_allocated(2)/1e9:.2f}GB")
    print(f"Cached: {torch.cuda.memory_reserved(2)/1e9:.2f}GB")
    
# Call this function at key points in your code
print_gpu_memory()

# Setup configurations
DATA_DIR = '/v/raid1b/backup/hhisham/MRIdata/Cardiac/DIP/data/my_kspace'
sub_name = 'P052924'
PLT_SAVE_FOLDER = '../figs/dip_results_3d'
current_setting = 'renewed_smap_normalized_all_coils_3d-self-guided-renewed-sensmap-kspace-no-vanilla'
...
for epoch in tqdm(range(no_of_epochs)):
    ...
    # prev
    # net_output = torch.zeros((1, 2, nz, ny, nx), device=device)
    # for _ in range(3):
    #     noise = noise_max * torch.rand_like(ref)
    #     net_output += net(ref + noise)
    # net_output_final = net_output / 3
    # prev
    random_smoothing_temp = torch.zeros_like(ref).to(device)
    # RUN MORE EXPECTED VALUES
    for jj in range(3):
        noise = noise_max * torch.randn(*ref.shape, device=device)   
    ...
    net_output_final = torch.view_as_complex(
        net_output_final.squeeze().permute(1, 2, 3, 0).contiguous())
    ...
    dx = torch.abs(net_output_final[..., 1:, :] - net_output_final[..., :-1, :])
    dy = torch.abs(net_output_final[..., :, 1:] - net_output_final[..., :, :-1])
    dz = torch.abs(net_output_final[1:, ...] - net_output_final[:-1, ...])

    tv_loss = lambda_tv * (torch.sum(dx) + torch.sum(dy) + torch.sum(dz))
    ...
    with torch.no_grad():
        ...
        if (epoch + 1) % show_every == 0:
            ...
            plt.plot(tv_losses)
            plt.title('TV Loss')
            plt.savefig(os.path.join(epoch_path, 'tv_loss.png'))
            plt.close()
            ...
            plt.plot(tv_losses, label='TV Loss')
            ...
            # save as 3d matrix (if epoch greater than 400 and divisible by 400)
    ...
    # Define header names
    header = 'loss,tv_loss,fidelity_loss'

    # Save losses with header
    np.savetxt(
        os.path.join(plt_save_path, 'losses.csv'),
        np.array([losses, tv_losses, fidelity_losses]).T,
        delimiter=',',
        header=header,  # Add header
    )
    # save all 3 losses in a csv file
    # np.savetxt(os.path.join(plt_save_path, 'losses.csv'), np.array([losses, tv_losses, fidelity_losses]).T, delimiter=',')
